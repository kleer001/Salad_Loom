# Salad Loom MCP Server

Enable LLMs to create and execute Salad Loom workflows via Model Context Protocol (MCP).

## Overview

The Salad Loom MCP server exposes Salad Loom's workflow capabilities to LLM agents like Claude. LLMs can:
...
```json
{
  "mcpServers": {
    "salad-loom": {
      "command": "/path/to/Salad_Loom/mcp_server",
      "args": []
    }
  }
}
...
**Returns**: Flowstate JSON that user can save and load in Salad Loom
...
4. User runs with `./salad_loom -b -f workflow.json`
...
- GitHub Issues: https://github.com/kleer001/Salad_Loom/issues
- Documentation: `/docs/`
