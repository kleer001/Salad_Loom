...
INSTALL_DIR="${INSTALL_DIR:-Salad_Loom}"
...
header() {
    echo -e "${CYAN}${BOLD}"
    echo "╔══════════════════════════════════════╗"
    echo "║      Salad Loom Installer v1.0       ║"
    echo "╚══════════════════════════════════════╝"
    echo -e "${NC}"
}
...
clone_repo() {
    step "Cloning Salad_Loom repository..."
    ...
    git clone https://github.com/kleer001/Salad_Loom.git "$INSTALL_DIR" || fail "Clone failed"
...
setup_venv() {
    ...
    python3 -m spacy download en_core_web_sm --quiet || fail "spaCy model download failed"
    ok "spaCy en_core_web_sm installed"

    step "Downloading NLTK data..."
    python3 -c "
import nltk
for pkg in ['averaged_perceptron_tagger_eng', 'punkt_tab', 'words', 'cmudict']:
    nltk.download(pkg, quiet=True)
" || fail "NLTK data download failed"
...
echo "Salad_Loom environment activated!"
echo "Run: ./salad_loom"
EOL
...
next_steps() {
    ...
    echo -e "  ${GREEN}cd $INSTALL_DIR && source activate.sh && ./salad_loom${NC}"
    echo
    echo -e "${CYAN}Available modes:${NC}"
    echo -e "  ${GREEN}./salad_loom -t${NC}  ${YELLOW}Terminal UI${NC}"
    echo -e "  ${GREEN}./salad_loom -r${NC}  ${YELLOW}Python REPL${NC}"
    echo -e "  ${GREEN}./salad_loom -g${NC}  ${YELLOW}Web GUI${NC}"
    echo -e "  ${GREEN}./salad_loom -b${NC}  ${YELLOW}Batch mode${NC}"
    echo
