...
## Why MCP for Salad Loom?

Salad Loom + MCP enables:
...
```json
{
  "mcpServers": {
    "salad-loom": {
      "command": "/absolute/path/to/Salad_Loom/mcp_server"
    }
  }
}
```
...
> "Using Salad Loom, create a workflow that reads article.txt, summarizes it with Claude, and saves to summary.txt"
...
└──────┬──────┘
       ...
       ↓
┌─────────────┐
│ Salad Loom   │ ← Tools: create_session, add_node, connect_nodes, etc.
│ MCP Server  │
└──────┬──────┘
       │ Python API
       ↓
┌─────────────┐
│ Salad Loom   │ ← Nodes, connections, execution
│    Core     │
└─────────────┘
```
...
async with Client("salad-loom") as client:
...
# Each session has:
{
  ...
  "workspace_file": "/tmp/salad_loom_sessions/uuid.json",
...
High-level API wrapping Salad Loom core:
...
```json
{
  "mcpServers": {
    "salad-loom": {
      "command": "/Users/alice/Salad_Loom/mcp_server"
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "salad-loom": {
      "command": "C:\\Users\\Alice\\Salad_Loom\\mcp_server"
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "salad-loom": {
      "command": "/home/alice/Salad_Loom/mcp_server"
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "salad-loom": {
      "command": "/path/to/Salad_Loom/.venv/bin/python",
      "args": ["/path/to/Salad_Loom/mcp_server"]
    }
  }
}
```
...
2. In correct directory? `pwd` should show Salad_Loom
...
https://github.com/kleer001/Salad_Loom/issues

---

**Ready to supercharge Salad Loom with LLM automation? Install the MCP server and let Claude build workflows for you!**
